...
function Install-ToolsIfNeeded {
    Write-StatusMessage "🔧 Checking and installing required tools..." "Info"
    ...
    foreach ($tool in $tools) {
        Write-StatusMessage "🔍 Checking $($tool.Name)..." "Info"
        
        if (Test-Command $tool.Check) {
            Write-StatusMessage "✅ $($tool.Name) is already installed" "Success"
        }
        else {
            Write-StatusMessage "⚠️  $($tool.Name) not found" "Warning"
            ...
            Write-StatusMessage "💡 Install with: $installCmd" "Highlight"
        }
    }
    
    # Python packages
    Write-StatusMessage "🐍 Checking Python development tools..." "Info"
    $pythonTools = @("ptpython", "ipython", "jupyter", "glances")
    
    foreach ($tool in $pythonTools) {
        try {
            pip show $tool 2>$null | Out-Null
            Write-StatusMessage "✅ $tool is installed" "Success"
        }
        catch {
            Write-StatusMessage "⚠️  $tool not found. Install with: pip install $tool" "Warning"
        }
    }
}

function Setup-StarshipConfig {
    Write-StatusMessage "🌟 Setting up Starship configuration..." "Info"
    ...
    if (Test-Path $sourceConfig) {
        ...
        Write-StatusMessage "✅ Starship configuration installed to $starshipConfig" "Success"
    }
    else {
        Write-StatusMessage "⚠️  Source config not found at $sourceConfig" "Warning"
    }
}

function Setup-PowerShellProfile {
    Write-StatusMessage "📝 Setting up PowerShell profile..." "Info"
...
if (Get-Command starship -ErrorAction SilentlyContinue) {
    ...
    Write-Host "⚠️  Starship not found. Install with: winget install starship" -ForegroundColor Yellow
}

# Load Super Alita development aliases if in project directory
if (Test-Path "scripts\dev-aliases.ps1") {
    . "scripts\dev-aliases.ps1"
    Write-Host "🚀 Super Alita development environment loaded!" -ForegroundColor Green
...
Write-Host ""
Write-Host "🎯 Super Alita Terminal Cockpit Ready!" -ForegroundColor Green
Write-Host "💡 Type 'alita-info' for development commands" -ForegroundColor Yellow
Write-Host ""
"@
    ...
    if (Test-Path $profilePath) {
        ...
        Write-StatusMessage "📋 Backed up existing profile to $backupPath" "Info"
    }
    
    # Add our content to profile
    Add-Content -Path $profilePath -Value $profileContent
    Write-StatusMessage "✅ PowerShell profile updated at $profilePath" "Success"
    Write-StatusMessage "🔄 Restart PowerShell or run '. $profilePath' to activate" "Info"
}

function Setup-DockerEnvironment {
    Write-StatusMessage "🐳 Setting up Docker development environment..." "Info"
    
    if (-not (Test-Command "docker")) {
        Write-StatusMessage "❌ Docker not found. Please install Docker first." "Error"
        return
    }
    
    # Verify docker-compose.yml exists
    if (-not (Test-Path "docker-compose.yml")) {
        Write-StatusMessage "❌ docker-compose.yml not found in current directory" "Error"
        return
    }
    
    Write-StatusMessage "🔍 Validating Docker Compose configuration..." "Info"
    try {
        docker-compose config | Out-Null
        Write-StatusMessage "✅ Docker Compose configuration is valid" "Success"
    }
    catch {
        Write-StatusMessage "❌ Docker Compose configuration error: $($_.Exception.Message)" "Error"
        return
    }
    
    Write-StatusMessage "🚀 Starting development stack..." "Info"
    docker-compose up -d
    
    Start-Sleep -Seconds 10
    
    Write-StatusMessage "📊 Stack status:" "Info"
    docker-compose ps
    
    Write-StatusMessage "🌐 Services available at:" "Success"
...
function Initialize-SecretsTemplate {
    Write-StatusMessage "🔐 Setting up secrets management..." "Info"
...
"@
    ...
    Write-StatusMessage "✅ Secrets template created: .env.secrets.template" "Success"
    
    if (-not (Test-Path ".env.secrets")) {
        Copy-Item ".env.secrets.template" ".env.secrets"
        Write-StatusMessage "📝 Created .env.secrets - EDIT WITH YOUR REAL VALUES" "Warning"
    }
    ...
    if (Test-Path ".gitignore") {
        ...
        if ($gitignore -notmatch "\.env\.secrets") {
            ...
            Write-StatusMessage "✅ Updated .gitignore with secrets exclusions" "Success"
        }
    }
    else {
        Set-Content ".gitignore" ($gitignoreEntries -join "`n")
        Write-StatusMessage "✅ Created .gitignore with secrets exclusions" "Success"
    }
}

function Show-CompletionMessage {
    Write-StatusMessage "" "Info"
    Write-StatusMessage "🎉 Ultimate Terminal Cockpit Setup Complete!" "Success"
    Write-StatusMessage "=" * 50 "Success"
    Write-StatusMessage "" "Info"
    Write-StatusMessage "🚀 Quick Start Commands:" "Highlight"
    ...
    Write-StatusMessage "" "Info"
    Write-StatusMessage "📚 Key Features Installed:" "Highlight"
    Write-Host "   ✅ Starship prompt with agent status indicators" -ForegroundColor Green
    Write-Host "   ✅ Docker development stack (Redis, Mongo, Neo4j, etc.)" -ForegroundColor Green
    Write-Host "   ✅ Enhanced PowerShell profile with shortcuts" -ForegroundColor Green
    Write-Host "   ✅ Secrets management template" -ForegroundColor Green
    Write-Host "   ✅ Professional development aliases" -ForegroundColor Green
    Write-StatusMessage "" "Info"
    Write-StatusMessage "🔄 Next Steps:" "Warning"
...
# Main execution
Write-StatusMessage "🚀 Super Alita Ultimate Terminal Cockpit Setup" "Highlight"
