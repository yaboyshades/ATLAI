#!/usr/bin/env pwsh
# Fix Pydantic v2 warnings and indentation issues

Write-Host "🔧 Starting Super Alita code fixes..." -ForegroundColor Green

# 1. Fix Pydantic v2 warnings
Write-Host "📝 Fixing Pydantic v2 configuration warnings..." -ForegroundColor Yellow

# Show files still using the old key
$oldKeyFiles = Select-String -Path src\**\*.py -Pattern "allow_population_by_field_name" -ErrorAction SilentlyContinue
if ($oldKeyFiles) {
    Write-Host "Found files with old Pydantic v1 config:" -ForegroundColor Red
    $oldKeyFiles | Select-Object Path, LineNumber, Line | Format-Table

    # Replace old key with v2 key
...
# Also fix validate_by_name if found
$validateFiles = Select-String -Path src\**\*.py -Pattern "validate_by_name" -ErrorAction SilentlyContinue
if ($validateFiles) {
    Write-Host "Found files with validate_by_name (should be populate_by_name):" -ForegroundColor Yellow
    (Get-ChildItem -Path src -Filter *.py -Recurse) | ForEach-Object {
        $content = Get-Content $_.FullName -Raw -ErrorAction SilentlyContinue
        if ($content -and $content.Contains("validate_by_name")) {
            Write-Host "Fixing validate_by_name in: $($_.FullName)" -ForegroundColor Cyan
            $content = $content -replace "validate_by_name", "populate_by_name"
            Set-Content $_.FullName $content -NoNewline
        }
    }
}
...
Write-Host "🔧 Fixing indentation issues..." -ForegroundColor Yellow

$conversationFile = "src\plugins\conversation_plugin.py"
if (Test-Path $conversationFile) {
    Write-Host "Converting tabs to spaces in: $conversationFile" -ForegroundColor Cyan
    $content = Get-Content $conversationFile -Raw
    # Convert tabs to 4 spaces
    $content = $content.Replace("`t", "    ")
    # Replace non-breaking spaces with regular spaces
    $content = $content.Replace([char]0x00A0, " ")
    Set-Content $conversationFile $content -NoNewline
}
...
# 4. Format the conversation plugin
Write-Host "🎨 Formatting conversation plugin..." -ForegroundColor Yellow
try {
    python -m black $conversationFile --quiet
    python -m ruff check --fix $conversationFile --quiet
    Write-Host "✅ Conversation plugin formatted" -ForegroundColor Green
}
catch {
    Write-Host "⚠️ Formatting failed: $($_.Exception.Message)" -ForegroundColor Red
}
...
Write-Host "🎉 Code fixes complete!" -ForegroundColor Green
Write-Host "Next steps:" -ForegroundColor Cyan
Write-Host "1. Run: python -c 'import src.plugins.conversation_plugin'" -ForegroundColor White
Write-Host "2. Test: python src/main.py" -ForegroundColor White

# SIG # Begin signature block
# MIIaSQYJKoZIhvcNAQcCoIIaOjCCGjYCAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBW9HluxMlFFVxr
# JZtnfZi49GjWONFFRMMTndcwBXWu+qCCFRswggHdMIIBRqADAgECAhB1LVe8LkJa
...
# MQ4wDAYKKwYBBAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQg8PPQto2vi9CzYE+lGLne
# d9sLkfGPAubC2kgA1DV8ysIwDQYJKoZIhvcNAQEBBQAEgYBTMcNd6KRaBYQsy+6N
# iAKAX/VUGB3zhAxEYuOYBmVxw1ocKtPcUxTK1vRJkJ/TfpGHM+0lxpV5U+7iz8xc
# tCPR6GECqGbbTxofjO98cUePngwz7NHIiN6OGjHPXmML4vPdXnHcIZcDCbOxA5nU
# xgvKvmO5bJ+PZgMK6cioXF/z0KGCAyYwggMiBgkqhkiG9w0BCQYxggMTMIIDDwIB
...
# MTkxOTI4MTVaMC8GCSqGSIb3DQEJBDEiBCCqOdhHfrCKj66Yfew35dFeU2EdN9zx
# jxl2i/b523w2JDANBgkqhkiG9w0BAQEFAASCAgAWDv3bGK+s2YSuHIhay2TGApHK
# ALCu9UkIryjihtYVxtIQ65npwYA2r3Dy0ygmHyPRFQ6jUcIo+N1+NtX9KwJh4nS4
# JVp2UrYQVfEZcPuMB7oSH2eXMyRqJXRF+WOYtWJLMVVwR1TKgLQrRC7EYPTvKhlO
# W8Ei/pkHAnEZf1A0cpEGWy17igQmkCttGSuEiZNgGk++cs4geoi/UYUmarP4uPFb
# xKSCwma/BNB6qoIKOp+TvAWKMWSKYC1CrohqgrfqLZvaL4+jHYc2h+pIyXf/qs7Y
# 1fUD0G6KW8solxvXGE7KmpdP3GdsV6W+IJF0H74nDS5ZypRNQZH5b9kaJfuUcTaR
# CYD0XNJLWYnLERzhZfS3PYCuuxfLsIZhfkZWZwIHl3M0uT2Wiz4ihuXxWj7Gz+YI
# 7OXi/7QcrapJuhzZrYpVxi7VmyqOQTJTBKSOCgXmbbGlg/LD7H+nBdDNWmo+3LcC
# wPFpB/R2xen7dwuCJRA6PdMGs1NEd+/7diwtV22xD9GuNq0CqPdd+6fozo5jEOdP
# LxXURm5PkHY5UMPqPCMF4Ggf7wrIfHhkDC4Ah7cewNy87wDVFSwyjYeBKsqK7YL6
# h8j/YroSHU/gZEwNx7fx1adqcqcbe/gWifRd2zbfEGuSFFJsiH2I4uq7AtMiLh9t
# oTzTc0FgNDza73MgRQ==
